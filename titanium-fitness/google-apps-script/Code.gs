 ...
 *   ADMIN_EMAIL      = destination address (defaults to fyblabs.web@gmail.com)
 *   ALLOWED_HOSTNAMES = comma-separated Turnstile hostnames
 */
...
function verifyTurnstile_(token) {
  ...
  try {
    ...
    const configured = properties.getProperty('ALLOWED_HOSTNAMES');
    const allowedHostnames = (configured
      ? configured.split(',')
      : SETTINGS.DEFAULT_ALLOWED_HOSTNAMES
...
function sendLeadEmails_(lead) {
  const properties = PropertiesService.getScriptProperties();
  const configuredAdmin = stringValue_(properties.getProperty('ADMIN_EMAIL')).trim();
  const adminEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(configuredAdmin)
    ? configuredAdmin
