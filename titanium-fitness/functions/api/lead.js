...
// Code.gs. Keeping the /exec URL here as an environment variable prevents it
// from being exposed in the browser or committed to GitHub.

const MAX_REQUEST_CHARS = 15_000;
const APPS_SCRIPT_URL_RE = /^https:\/\/script\.google\.com\/macros\/s\/[^/]+\/exec$/;

export function onRequestGet() {
  return json({ ok: true, service: 'Titanium Fit lead bridge' });
}

export async function onRequestPost(context) {
  const { request, env } = context;
  const appsScriptUrl = env.APPS_SCRIPT_WEB_APP_URL || '';

  if (!APPS_SCRIPT_URL_RE.test(appsScriptUrl)) {
