...
REM  Solo miran los archivos YA trackeados (-uno): los archivos
REM  nuevos sin agregar, como este mismo .bat, no estorban a un
REM  rebase y no deben frenarlo.
for /f %%i in ('git status --porcelain -uno 2^>nul ^| find /c /v ""') do set SUELTOS=%%i
if not "!SUELTOS!"=="0" (
